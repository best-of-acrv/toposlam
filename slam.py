...
from loader.kitti import KITTIOdom as Loader
...
from drawer.frame_drawer import FrameDrawer
...
class SLAM():
    def __init__(self, img_dir, calib_dir, cfg, device):
        ...
        self.loader = Loader(self.img_dir, self.calib_dir, self.height, self.width, self.ext)
        self.img_id_list = self.loader.get_id_list()
        self.K = self.loader.get_intrinsics_param()  # camera intrinsic: [cx, cy, fx, fy]

        # setup DFVO module
        self.dfvo = DFVO_Mod(self.cfg, self.K)

        # setup loop closure detection module
        self.loop_closure_detect = LoopClosureDetect(self.device, self.cfg.net_vlad.pretrained_model)
    ...
    def update_global_pose(self, new_pose, img_id, scale=1.):
        ...
        """
        self.cur_data['pose'].t = self.cur_data['pose'].R @ new_pose.t * scale + self.cur_data['pose'].t
    ...
    def main(self):
        ...
        for img_id in tqdm(range(start_frame, len(self.img_id_list), self.cfg.frame_step)):

            if self.tracking_stage == 0:  # the starting frame
                self.cur_data['id'] = self.img_id_list[img_id]
                self.cur_data['img'] = self.loader.get_image(self.cur_data['id'])
                self.cur_data['pose'] = SE3()

                # pose graph
                self.pose_graph_optimize.add_vertex(img_id,
                                                    g2o.Isometry3d(self.cur_data['pose'].R, self.cur_data['pose'].t),
                                                    True)

                # loop closure detection, adding the current frame to the database
                _ = self.loop_closure_detect([self.cur_data['img'].transpose(2, 0, 1)], [img_id], False)
                ...
                valid, new_pose, depth, flow = self.dfvo.main(self.ref_data['img'], self.cur_data['img'])
                if valid:
                ...
                self.global_poses_opt[img_id] = copy.deepcopy(self.global_poses_pred[img_id])
                ...
                self.pose_graph_optimize.add_edge([img_id - 1, img_id],
                                                  g2o.Isometry3d(new_pose.R, new_pose.t))

                # loop closure detection
                lcd_result = self.loop_closure_detect([self.cur_data['img'].transpose(2, 0, 1)], [img_id], True)

                # add new edge to the pose graph if loop closure is detected
                if lcd_result[0][0] is not None:
                    print('loop closure detection: matched pair of image {} and {} '
                          'with the belief of {:06f}\n'.format(lcd_result[0][1], lcd_result[0][0], lcd_result[0][2]))

                    self.mat_data['id'] = self.img_id_list[int(lcd_result[0][1])]
                    # self.mat_data['query_id'] = copy.copy(self.cur_data['id'])
                    self.mat_data['img'] = self.loader.get_image(self.mat_data['id'])

                    self.lc_pairs.append([int(lcd_result[0][1]), int(lcd_result[0][0])])
                    valid, lcd_pose, _, _ = self.dfvo.main(self.cur_data['img'], self.mat_data['img'])

                    if valid:
                        # adding edge to the pose graph
                        self.pose_graph_optimize.add_edge([img_id, int(lcd_result[0][1])],
                        ...
                        for img_id_opt in range(start_frame + self.cfg.frame_step, img_id + 1, self.cfg.frame_step):
                            self.global_poses_opt[img_id_opt] = SE3(self.pose_graph_optimize.get_pose(img_id_opt))
                    else:
                        continue

                # visualization
                self.drawer.main(self.cur_data['img'], self.cur_data['depth'], self.cur_data['flow'],
                             self.global_poses_pred, self.global_poses_opt, self.lc_pairs)

            # update reference and current data
            self.ref_data, self.cur_data = self.update_data(self.ref_data, self.cur_data)
