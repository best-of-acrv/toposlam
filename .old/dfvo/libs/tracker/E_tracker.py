...
from dfvo.libs.geometry.ops_3d import *
from dfvo.libs.geometry.rigid_flow import RigidFlow
from dfvo.libs.general.utils import image_shape, image_grid
from dfvo.libs.matching.kp_selection import opt_rigid_flow_kp
...
class EssTracker():
    def __init__(self, cfg, cam_intrinsics, timers=None):
        """
        Args:
    ...
    def compute_pose_2d2d(self, kp_ref, kp_cur, is_iterative):
        ...
        if valid_cfg.method == "flow":
            ...
            if kp_cur.shape[0] > 10:
                ...
                if self.timers: self.timers.end('GRIC-H')
            else:
                valid_case = False
        ...
        if valid_case:
            num_valid_case = 0
            if self.timers: self.timers.start('find-Ess (full)', 'E-tracker')
            for i in range(max_ransac_iter): # repeat ransac for several times for stable result
                ...
                if self.timers: self.timers.start('find-Ess', 'E-tracker')
                E, inliers = cv2.findEssentialMat(
                            new_kp_cur,
                            new_kp_ref,
                ...
                if self.timers: self.timers.end('find-Ess')
                ...
                if valid_cfg.method == "homo_ratio":
                    ...
                    if self.timers: self.timers.start('GRIC-E', 'E-tracker')
                    # get F from E
                    ...
                    # inlier check
                    inlier_check = inliers.sum() > best_inlier_cnt
                    if self.timers: self.timers.end('GRIC-E')
            ...
            if self.timers: self.timers.end('find-Ess (full)')
            major_valid = num_valid_case > (max_ransac_iter/2)
            if major_valid:
                if self.timers: self.timers.start('recover pose', 'E-tracker')
                cheirality_cnt, R, t, _ = cv2.recoverPose(best_E, kp_cur, kp_ref,
                                        focal=self.cam_intrinsics.fx,
                                        pp=principal_points,
                                        )
                if self.timers: self.timers.end('recover pose')
    ...
    #     elif valid_cfg.method == "GRIC":
    #         if self.timers: self.timers.start('GRIC-H', 'E-tracker')
    #         if self.timers: self.timers.start('find H', 'E-tracker')
    #         H, H_inliers = cv2.findHomography(
    #                     kp_cur,
    #                     kp_ref,
    #                     method=cv2.RANSAC,
    ...
    #                     ransacReprojThreshold=1,
    #                     )
    #         if self.timers: self.timers.end('find H')
    ...
    def find_scale_from_depth(self, kp1, kp2, T_21, depth2):
        ...
        if self.timers: self.timers.start('triangulation', 'scale_recovery')
        ...
        if self.timers: self.timers.end('triangulation')
        ...
        if valid_mask2.sum() > 10:
            ...
            if self.timers: self.timers.start('scale ransac', 'scale_recovery')
            ...
            if self.timers: self.timers.end('scale ransac')
